...
function q(a,c,b,d,f){
    var g = a.find("img");
    if (g[0].complete && 50 < g[0].naturalWidth && 50 < g[0].naturalHeight)
        r(g,a,c,b,d,f);
    else
        g.one("load", function(){
            r(g,a,c,b,d,f);
...
function r(a,c,b,d,f,g){
    ...
    d = d >= l ? l : d;
    b = (b - d*p/l) / 2;
    a.css({ width: parseInt(d), height: d*p/l });
    c.css("top", b + f);
}
...
function z(a,c,e,d,f,g){
    a.find(".modal-dialog .carousel-item").each(function(){
        if (
            (!g && !b(this)[0].classList.contains("carousel-item-next") &&
...
function t(){
    ...
    if (h){
        ...
        h.find(".modal-dialog .carousel-item.carousel-item-next, .modal-dialog .carousel-item.carousel-item-prev").length
...
if (!k){
    ...
    // ---------------------------------------------------------
    // 🔒 BLOQUEO DE YOUTUBE HASTA ACEPTAR COOKIES
    // ---------------------------------------------------------

    var v = document.createElement("script");
    v.type = "text/plain";                 // ← bloqueado
    v.setAttribute("data-cookie", "youtube");
    v.src = "https://www.youtube.com/iframe_api";

    // NO insertar todavía
    // Se insertará cuando el usuario pulse "Aceptar"
    // mediante enableBlockedScripts()
    ...
    window.onYouTubeIframeAPIReady = function(){
        ...
        b(".video-slide").each(function(a){
            b(".video-container").eq(a)
                .append('<div id="mbr-video-'+a+'" class="mbr-background-video" data-video-num="'+a+'"></div>')
...
b(document).on("add.cards", function(a){
    ...
    c.on("click", ".mbr-gallery-filter li", function(a){
        ...
        c.find(".mbr-gallery-item").each(function(a,c){
            var e = b(this),
                h = e.attr("data-tags").split(",").map(function(a){ return a.trim(); });
...
if (k)
    b(document).on("changeButtonColor.cards", function(a){
        ...
        {
            var e = (c.find(".mbr-gallery-filter .mbr-gallery-filter-all")
                        .find("a").attr("class") || "")
                        .replace(/(^|\s)active(\s|$)/, " ")
                        .trim();

            c.find(".mbr-gallery-filter ul li:not(.mbr-gallery-filter-all) a")
                .attr("class", e);
        }
        m(a);
    });
